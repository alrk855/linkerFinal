...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const NAV_ITEMS = [
    ...
    { label: "Students", href: "/en/admin/students", icon: Users },
    { label: "Companies", href: "/en/admin/companies", icon: Building2 },
  ];
    ...
    <div className="flex w-full min-h-screen bg-background">
      ...
      <aside className="w-64 bg-surface border-r border-border flex flex-col hidden md:flex shrink-0">
        <div className="h-16 flex items-center px-6 border-b border-border">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded bg-accent flex items-center justify-center text-background font-bold text-[10px]">
              L
            </div>
            <span className="font-semibold tracking-tight text-foreground">Linker Admin</span>
          </div>
        </div>
        ...
        <nav className="flex-1 py-6 px-4 space-y-1">
          {NAV_ITEMS.map(item => {
            ...
            const isActive = pathname === item.href || (pathname.startsWith(item.href) && item.href !== "/en/admin");
            return (
      ...
      <main className="flex-1 min-w-0 flex flex-col h-screen overflow-y-auto">
        <header className="h-16 border-b border-border bg-surface flex items-center justify-end px-8 md:hidden">
          <span className="font-semibold px-4">Linker Admin</span>
        </header>
